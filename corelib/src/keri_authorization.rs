...
impl KeriWrapper {
    ...
    pub(crate) fn load_identifier(config_file: &Path) -> Result<KeriWrapper, AuthenticationError> {
        ...
        identifier.configure_watcher(&config.watcher_oobi[0])?;
        identifier.configure_witness(&config.witness_oobi)?;
        identifier.configure_messagebox(&config.messagebox_oobi[0])?;

        Ok(identifier)
    }

    fn configure_watcher(&self, watcher_oobi: &LocationScheme) -> Result<(), AuthenticationError> {
    ...
    fn configure_messagebox(&self, oobi: &LocationScheme) -> Result<(), AuthenticationError> {
        let rt = Runtime::new().unwrap();
        rt.block_on(async {
            self.controller
                .source
    ...
    pub(crate) fn resolve_oobi(&mut self, signer_oobi: &Oobi) -> Result<(), AuthenticationError> {
        let rt = Runtime::new()?;
    ...
    pub(crate) fn verify_stream(
        ...
        if !stream.is_empty() {
            ...
            creds
                ...
                .unwrap()
                .iter()
                .try_for_each(|cred| self.check_tel(cred.as_bytes()))?;
            creds
        } else {
    ...
    fn check_tel(&self, cred: &[u8]) -> Result<(), AuthenticationError> {
        let rt = Runtime::new()?;
    ...
    pub fn get_data(&self) -> Result<String, AuthenticationError> {
